export const environment = {
  title: 'stigoRXOLD',
  medicines_url: 'https://julyfifteenapi.onrender.com/api/medicines',
  clients_url: 'https://julyfifteenapi.onrender.com/api/clients',
  inventories_url: 'https://julyfifteenapi.onrender.com/api/inventories',
  stores_url: 'https://julyfifteenapi.onrender.com/api/stores',
  units_url: 'https://julyfifteenapi.onrender.com/api/units',
  suppliers_url: 'https://julyfifteenapi.onrender.com/api/suppliers',
};
